...
float perlin1dTo1d(float value, float mutator = 0.546) {
    float fraction = frac(value);
    float interpolator = smoothstep(0, 1, fraction);

    float previousCellInclination = rand1dTo1d(floor(value), mutator) * 2 - 1;
    float previousCellLinePoint = previousCellInclination * fraction;

    float nextCellInclination = rand1dTo1d(ceil(value), mutator) * 2 - 1;
...
float perlinNoise(float2 value){
    ...
    float interpolatorX = smoothstep(0, 1, fraction.x);
    float interpolatorY = smoothstep(0, 1, fraction.y);
